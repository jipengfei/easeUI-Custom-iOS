...
- (void)scrollToBottom:(BOOL)animated {
    [self _scrollViewToBottom:animated];
}
...
- (void)didStartRecordingVoiceAction:(UIView *)recordView
{
    ...
    [self _canRecordCompletion:^(EMRecordResponse recordResponse) {
        switch (recordResponse) {
            ...
            case EMCanNotRecord:
            {
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"去开启访问麦克风权限?" message:nil preferredStyle:UIAlertControllerStyleAlert];
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (model) {
            ...
            if (model.isSenderInBlackList) {
                NSString *failString = @"消息已发出，但被拒收了";
                [formattedArray addObject:failString];
            }
        }
    }
...
-(void)reloadMessageToDataSource:(EMMessage *)message
{
    __weak EaseMessageViewController *weakSelf = self;
    dispatch_async(_messageQueue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            id lastModel = [weakSelf.dataArray lastObject];
            id<IMessageModel> model = nil;
            if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
                model = [_dataSource messageViewController:self modelForMessage:message];
            }
            
            if ([lastModel isKindOfClass:[EaseMessageModel class]]) {
                if (model.isSenderInBlackList) {
                    NSString *failString = @"消息已发出，但被拒收了";
                    [weakSelf.dataArray addObject:failString];
                }
            }else if([lastModel isKindOfClass:[NSString class]]){
                NSString *str = lastModel;
                if ([str isEqualToString:@"消息已发出，但被拒收了"]) {
                    
                    if (model && !model.isSenderInBlackList) {
