...
- (void)_willShowBottomView:(UIView *)bottomView
{
    if (![self.activityButtomView isEqual:bottomView]) {
        ...
        if (self.usedByPrivateMessage) {
            if (bottomHeight > 0) {
                if (self.delegate && [self.delegate respondsToSelector:@selector(willShowCustomKeyboard:withHeight:)]) {
                    [self.delegate willShowCustomKeyboard:YES withHeight:bottomHeight];
                }
                self.keyboardType = SYShowKeyboardType_Custom;
            } else {
                if (self.delegate && [self.delegate respondsToSelector:@selector(willShowCustomKeyboard:withHeight:)]) {
                    [self.delegate willShowCustomKeyboard:NO withHeight:bottomHeight];
                }
                self.keyboardType = SYShowKeyboardType_None;
            }
        }
...
- (void)_willShowKeyboardFromFrame:(CGRect)beginFrame toFrame:(CGRect)toFrame
{
    ...
    {
        self.keyboardType = SYShowKeyboardType_System;
